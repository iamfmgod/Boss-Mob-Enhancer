...
public class ConfigHandler {
    ...
    public static void init(File configFile) {
        ...
        difficultyPreset = config.getString(
                ...
                "Choose a scaling profile: Balanced, Aggressive, Brutal, SkyblockFriendly, SoloSurvivor, Raider, BerserkerRush, Endwalker."
        );
        config.getCategory("Scaling").get("Difficulty Curve Preset").setValidValues(new String[]{
        ...
        lootDropMultiplier = config.getFloat(
                "Loot Drop Multiplier", "Rewards", 0.25f, 0f, 1f,
                "Multiplier to reduce loot drop amounts for non-lord bosses (1.0 means full rewards)."
        );
        dungeonLootTables = config.getStringList(
                "Dungeon Loot Tables", "Rewards", new String[]{
                        "minecraft:chests/stronghold_library",
                        "minecraft:chests/nether_bridge",
                        "minecraft:chests/end_city_treasure"
                },
                "List of loot tables for dungeon loot rewards."
        );
        customLootTables = config.getStringList(
                "Custom Loot Tables", "Rewards", new String[]{},
                "List of custom loot table resource locations to use in boss drops."
        );
        customLootItems = config.getStringList(
                "Custom Loot Items", "Rewards", new String[]{},
                "Custom item drops in the format modid:itemname,quantity,chance"
        );
