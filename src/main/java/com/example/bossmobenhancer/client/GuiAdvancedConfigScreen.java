...
public class GuiAdvancedConfigScreen extends GuiConfig {
    ...
    private static List<IConfigElement> getAdvancedConfigElements() {
        ...
        // Set comments directly on the underlying categories.
        ConfigHandler.getConfig().getCategory("Scaling").setComment("Difficulty Scaling: Adjust internal multipliers and weights for dynamic boss scaling.");
        list.add(new ConfigElement(ConfigHandler.getConfig().getCategory("Scaling")));

        ConfigHandler.getConfig().getCategory("Abilities").setComment("Abilities & Effects: Enable and fine-tune special boss abilities and terrain effects.");
        list.add(new ConfigElement(ConfigHandler.getConfig().getCategory("Abilities")));

        ConfigHandler.getConfig().getCategory("Entities").setComment("Mob Filters: Specify whitelists and blacklists for enhanced bosses.");
        list.add(new ConfigElement(ConfigHandler.getConfig().getCategory("Entities")));

        ConfigHandler.getConfig().getCategory("Minions").setComment("Minion Settings: Adjust the spawn chance and maximum minions for bosses.");
    ...
    private void resetToDefaults() {
        ...
        // Reset Entities.
        ConfigHandler.getConfig().get("Entities", "whitelistMobs", new String[]{}).set(new String[]{});
        ConfigHandler.getConfig().get("Entities", "blacklistMobs", new String[]{}).set(new String[]{});
